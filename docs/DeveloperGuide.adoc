...
The sequence diagram below demonstrates the interaction within the `Logic` component of `SetScheduleCommand`:
...
** This function checks if the user is logged in at the same time.
...
|`* * *` |User	|Filter out and view my relevant colleague’s contact details	|Spend less time searching for their contact details and contact them
